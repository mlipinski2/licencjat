...
Budowa systemu oświetlenia, miała na celu znaczące obniżenie kosztów energii elektrycznej oraz prostotę i użyteczność które to wspierałyby nasze \emph{wrodzone lenistwo} we wszystkich pokojach w całym mieszkaniu wliczając łazienkę i  korytarz. Jak mawiał klasyk, \emph{"Do zrobienia rzeczy trudnej wybrałbym osobę leniwą, gdyż taka to właśnie osoba znajdzie najprostszy sposób na wykonanie tego} \cite{gates}. Do tego celu postanowiliśmy użyć wydajnych i energooszczędnych taśm LED'owych w połączeniu ze sterownikami WiFi które będą duszą naszego projektu. Całośc będzie sterowana zdalnie z dwu źródeł, sterownika stacjonarnego oraz/i aplikacji zainstalowanej na smartfonie. Tradycyjnie w celu obniżenia kosztów przedsięwzięcia wszystkie komponenty zostały zakupione u źródła czyli w centrum światowej produkcji Chinach na portalu \emph{Aliexpress.com}.
W skład wszyskich potrzebnych do budowy zdalnego sterowania oświetleniem komponentów wchodzą:
\begin{itemize}
	\item Taśmy LED'owe \emph{Warm yellow light} 6 taśm po 5m każda -> 30\$
